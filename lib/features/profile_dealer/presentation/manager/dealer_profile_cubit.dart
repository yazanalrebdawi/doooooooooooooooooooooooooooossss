...
class DealerProfileCubit extends Cubit<DealerProfileState> {
    ...
    result.fold(
      (failure) {
        ...
        if (failure.message.contains('401') ||
            failure.message.contains('Authentication')) {
          emit(
            state.copyWith(
              isLoading: false,
      ...
      (dealer) {
        emit(state.copyWith(isLoading: false, dealer: dealer));
      },
    );
  }
    ...
    result.fold(
      (failure) {
        ...
        if (failure.message.contains('401') ||
          ...
          emit(state.copyWith(isLoadingReels: false, error: failure.message));
        }
      },
      (reels) {
        emit(state.copyWith(isLoadingReels: false, reels: reels));
      },
    );
  }
  ...
  void loadCars(String dealerId) async {
    ...
    result.fold(
      (failure) {
        emit(state.copyWith(isLoadingCars: false, error: failure.message));
      },
      (cars) {
        emit(state.copyWith(isLoadingCars: false, cars: cars));
      },
    );
  }
